...
def convert_with_links(rpc_rule, fields=None, sanitize=True):
    ...
    inspection_rule['actions'] = list(
        convert_actions(rpc_rule.actions))
    inspection_rule['conditions'] = list(
        convert_conditions(rpc_rule.conditions))
    ...
    inspection_rule['built_in'] = False
...
class InspectionRuleController(rest.RestController):
    ...
    invalid_sort_key_list = ['actions', 'conditons']
    ...
    def get_all(self, marker=None, limit=None, sort_key='id', sort_dir='asc',
        ...
        """
        api_utils.check_policy('baremetal:inspection_rule:list')
    ...
    def post(self, inspection_rule):
        ...
        validation.validate_inspection_rule(inspection_rule)
    ...
    def patch(self, inspection_rule_uuid, patch=None):
        ...
        if 'built_in' in patch:
            msg = _("The 'built_in' field is immutable.")
            raise exception.PatchError(patch=patch, reason=msg)
    ...
    @args.validate(inspection_rule_uuid=args.uuid_or_name)
    def delete(self, inspection_rule_uuid):
        """Delete an inspection rule or all.

        :param inspection_rule_uuid: UUID of an inspection rule.
        :param confirm: Confirmation string. Must be 'true' for bulk deletion.
        """
        context = api.request.context
        if inspection_rule_uuid.lower() == 'all':
            api_utils.check_policy('baremetal:inspection_rule:delete_all')
            rules = objects.InspectionRule.list(context)
            if len(rules) == 0:
                raise webob_exc.HTTPNotFound(
                    _("No inspection rules found for deletion."))

            notify.emit_start_notification(context, rules[0], 'delete_all')
            with notify.handle_error_notification(context, rules[0],
                                                  'delete_all'):
                objects.InspectionRule.destroy_all()
            notify.emit_end_notification(context, rules[0], 'delete_all')
        else:
